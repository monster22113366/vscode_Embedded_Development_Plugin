...
### 🚀 一键开发流程
- **扫描工程**（F6）：自动识别芯片型号、调试器类型、生成配置；已扫描过的工程仅更新 CMakeLists.txt，速度更快
- **格式化代码**（Shift+F6）：手动触发 clang-format，独立于扫描流程
- **编译项目**（F7）：增量编译，实时显示进度
- **清理重建**（F9）：删除 build 目录后从零完整构建，解决缓存污染问题
- **烧录固件**（F8）：一键烧录到芯片
...
下载embedded-c-toolchain-2.0.0.vsix，从vscode导入。
...
|--------|------|------|
| **F5** | 启动调试 | 需先编译和连接调试器 |
| **F6** | 扫描工程 | 自动配置芯片和调试器；已扫描过仅更新 CMakeLists.txt |
| **Shift+F6** | 格式化代码 | 手动触发 clang-format，独立于扫描 |
| **F7** | 编译项目 | 增量编译，只编译修改的文件 |
| **F8** | 烧录固件 | 将编译后的 .bin 烧录到芯片 |
| **F9** | 清理重建 | 删除 build 目录后完整重新编译 |
...
3. 若怀疑编译缓存污染，按 **F9** 清理重建

### 使用 Keil MDK 工程？
直接按 F6 扫描，插件会自动解析 `.uvprojx` 文件，提取源文件、头文件路径和宏定义，无需手动配置。

### 使用 STM32CubeMX 生成的工程？
直接按 F6 扫描即可，插件会自动修复 CubeMX 新版本 CMakeLists.txt 中已知的头文件路径注释 bug。
